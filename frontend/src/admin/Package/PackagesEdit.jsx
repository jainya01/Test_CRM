...
function PackagesEdit() {
  ...
  const handleFormSubmit = async () => {
    try {
      ...
      await axios.put(`${API_URL}/packagesupdate/${id}`, formData, {
        headers: {
          ...authHeader(),
  ...
  useEffect(() => {
    const allData = async () => {
      try {
        const [packageRes] = await Promise.allSettled([
          axios.get(`${API_URL}/somepackages/${id}`, {
            headers: authHeader(),
          }),
        ]);

        if (packageRes.status === "fulfilled") {
          setPackages(packageRes.value.data.result);
        }
      } catch (error) {
        console.error(error);
      }
